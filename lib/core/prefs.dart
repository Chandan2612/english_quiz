...
class Prefs {
  ...
  static String stageKey(int round) => 'stage_round_$round';
  static String selectedKey(int round) => 'selected_round_$round';
  static String resultKey(int round) => 'result_round_$round'; // 👈 NEW
  ...
  static Future<void> clearRoundState(int round) async {
    ...
    await prefs.remove(stageKey(round));
    await prefs.remove(selectedKey(round));
    // note: we **do not** remove result here; users can still view it later
  }

  // ---------- NEW: store + read round result ----------
  static Future<void> setRoundResult(int round, int correct, int total) async {
    final prefs = await SharedPreferences.getInstance();
    final payload = jsonEncode({'c': correct, 't': total});
    await prefs.setString(resultKey(round), payload);
  }

  static Future<Map<String, int>?> getRoundResult(int round) async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString(resultKey(round));
