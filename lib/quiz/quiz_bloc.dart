...
import '../core/prefs.dart';
import 'models.dart';
import 'quiz_repository.dart'; // <-- brings in kQuestionsPerRound
...
class QuizBloc extends Bloc<QuizEvent, QuizState> {
  ...
  Future<void> _onLoadRound(LoadRound e, Emitter<QuizState> emit) async {
    ...
    // Stage might be from old 10-question runs. Clamp to 0..(kQuestionsPerRound-1)
    final persistedStage = await Prefs.getStage(round);
    final clampedStage = persistedStage.clamp(0, qs.length - 1);
    ...
    // Keep storage consistent if it was out of range
    if (persistedStage != clampedStage) {
      await Prefs.setStage(round, clampedStage);
    }
  }
