...
// // import '../widgets/ad_overlay.dart'; // uses AdMob interstitials (with fallback)
...
// //       listener: (context, state) async {
// //         // Auto-advance after a selection
// //         if (_lastSelectionTick != state.selectionTick) {
// //           _lastSelectionTick = state.selectionTick;
// //           _green('Selected ✨');
// //           final lastIdx = state.questions.length - 1;
...
// //         // Keep PageView in sync with Bloc index
// //         if (_controller.hasClients && !_isAnimating) {
// //           _isAnimating = true;
// //           final target = state.currentIndex;
// //           await _controller.animateToPage(
...
// //         // Submission flow
// //         if (state.submitted) {
// //           // 1) Show result bottom sheet
// //           await _showResult(context, state.correctCount, state.questions.length);

// //           // 2) Show 1 or 2 interstitials (falls back to dialog if load fails)
// //           await AdOverlay.showInterstitialTimes(
// //             context,
// //             times: state.adsToShow,
// //           );

// //           // 3) Advance to next round and return
// //           final next = (state.round + 1).clamp(1, 500);
// //           context.read<QuizBloc>().add(ResetForNextRound(next));
...
// //         final atFirst = state.currentIndex == 0;
// //         final atLast = state.currentIndex == totalQ - 1;

// //         return Scaffold(
// //           appBar: AppBar(
// //             title: Text('Round ${state.round} • Q${state.currentIndex + 1}/$totalQ'),
// //             actions: [
// //               IconButton(
// //                 tooltip: 'Previous',
// //                 onPressed: atFirst ? null : () => context.read<QuizBloc>().add(PrevQuestion()),
...
// //             Expanded(
// //               child: ElevatedButton(
// //                 onPressed: allAnswered && !state.submitted
// //                     ? () => context.read<QuizBloc>().add(SubmitRound())
// //                     : null,
// //                 child: const Text('Submit Round'),
// //               ),
// //             ),
...
//   void initState() {
//     super.initState();
//     // Start loading an interstitial early so it’s ready by the end.
...
//         // After submit → show result, ads, then go to next round
//         if (state.submitted) {
//           await _showResult(context, state.correctCount, state.questions.length);

//           // Show the required number of interstitials (await dismissal each time)
//           for (int i = 0; i < state.adsToShow; i++) {
//             await InterstitialManager.showAndWait();
//           }
...
//         return Scaffold(
//           appBar: AppBar(
//             title: Text('Round ${state.round} • Q${state.currentIndex + 1}/${state.questions.length}'),
//             actions: [
//               IconButton(
//                 tooltip: 'Previous',
//                 onPressed: atFirst ? null : () => context.read<QuizBloc>().add(PrevQuestion()),
...
class _RoundPageState extends State<RoundPage> {
  final _controller = PageController();
  final _messengerKey = GlobalKey<ScaffoldMessengerState>(); // ⬅️ local messenger
  int _lastSelectionTick = -1;
  bool _navLocked = false;
  bool _isAnimating = false;
  ...
  @override
  void dispose() {
    _messengerKey.currentState?.clearSnackBars(); // ⬅️ ensure nothing leaks
    _controller.dispose();
    super.dispose();
  }
  ...
  void _green(String msg) {
    ...
    if (m == null) return;
    m.hideCurrentSnackBar(); // ⬅️ avoid queueing
    m.showSnackBar(
      SnackBar(
        backgroundColor: Colors.green,
        content: Text(msg),
  ...
  Widget build(BuildContext context) {
    return ScaffoldMessenger(
      // ⬅️ scope SnackBars to this page only
      key: _messengerKey,
      child: BlocConsumer<QuizBloc, QuizState>(
        ...
        listener: (context, state) async {
          ...
          // Submit -> results -> ads -> next round -> pop
          if (state.submitted) {
            // ⬅️ clear any queued SnackBars before overlays/ads
            ...
            // ⬅️ clear again just before leaving
            _messengerKey.currentState?.clearSnackBars();
